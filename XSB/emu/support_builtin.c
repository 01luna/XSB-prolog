...
xsbBool support_builtin(CTXTdecl)
{
  ...
  case RETURN_E: {
    ...
    VariantSF sf = get_call(ptoc_tag(CTXTc 2), &ret);
    ...
    if ( IsNonNULL(sf) && IsNonNULL(sf->base_sf)) {
      ...
      printf("3\n");
      bottomupunify(ptoc_tag(CTXTc 3), root, leaf);
      printf("4\n"); 
    }
    break;
  }
  ...
  case GET_DIRTY_COUNT: {
    ...
    const int regCallTerm = 2;   /* in:  tabled call to look for */
    VariantSF sf;
    
    sf = get_dirty_count(ptoc_tag(CTXTc regCallTerm), &dirty_count);
    if ( IsNonNULL(sf) ) {
  ...
  case FAST_TRIE_INTERN: {
    fast_intern(ptoc_tag(CTXTc 1));
    break;
  }

  case GET_MODE: {
    //printf("In get_mode()?\n");
    int no_regs = ptoc_int(CTXTc 2);
    int maplen = 0,map = 0,temp;
    Cell term = ptoc_tag(CTXTc 3);
