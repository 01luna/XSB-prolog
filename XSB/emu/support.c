...
int search_in_term(Cell term, Cell c, int *p)
{
  ...
  case XSB_REF1: {
    *p=1;
    if (compare(CTXTc (void *)term, (void *)c)==0) {
      flag = 1;
    }
    else {
      flag = 0;
...
{
  sfPrintGoal(CTXTc stdout,ss->answer_of_subg,NO);
  sfPrintGoal(CTXTc stdout,ss->dynamic_answer_subg,NO);
}
...
void printornode(ornodeptr ans)
{
  ...
  if(subg!=NULL){
    printf("goal(");
    sfPrintGoal(CTXTc stdout,subg,NO);printf(",");
...
setsupportnodeptr makesetsupportnode(VariantSF subg)
{
  ...
  {
    printf("<%d>",no_ssnode);
    sfPrintGoal(CTXTc stdout,subg,NO);
    printf("<%d>\n",no_ssnode);
...
void adjust()
{
  ...
  for(ptr=delta_assumption;IsNonNULL(ptr);ptr=next) {
	...
	ss=ssl->item;
#ifdef DIPTI_DEBUG
	sfPrintGoal(CTXTc stdout,ss->dynamic_answer_subg,NO);printf("\n\n");
#endif	
