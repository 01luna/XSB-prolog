...
\standarditem{load\_dync(+FileName)}{load\_dync/1} \index{canonical format}
%
...
%    As usual, clauses
%    of predicates are not retracted if they are compiled instead of
%    dynamically asserted. All predicates are loaded into {\tt
%      usermod}.  {\tt :- export} declarations are ignored and a
%    warning is issued.
...
\standarditem{load\_dync(+FileName,+Ordering)}{load\_dync/2} \label{load_dync/2}
%
...
Acts as {\tt load\_dync/2}, but loads {\tt FileName} into the module named
{\tt Module}.

\standarditem{load\_dynca(+FileName)}{load\_dynca/1} \index{canonical format}
%
Convenience predicate for {\tt load\_dync(FileName,a)}: the fastest
dynamic loading in XSB.
