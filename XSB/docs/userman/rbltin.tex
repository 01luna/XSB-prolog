...
-- they can be asserted or retracted.  Without this declaration
compiled clauses will be treated as static.  Executed as a directive
in a state of execution where no clauses exist for each denoted
predicate {\tt dynamic/1} ensures clauses for the affected predicates
are to be treated as dynamic.  If {\tt PredSpec} is previously defined
as dynamic, {\tt dynamic/1} has no effect.  If {\tt PredSpec} was
previously defined as static, a permission error will be thrown.  {\tt
  Operations} can take one of two forms:
%
...
%
\item {\tt Options} is either a dynamic option or a list of dynamic
...
\item{\tt tabled} which causes the dynamic predicate to be tabled.
  The declaration/directive {\tt dynamic p/n as tabled} is the same as
  {\tt table p/n as dynamic}.
%
...
\item {\tt instantiation\_error}
\ei
\item {\tt Operations} is of the form {\tt PredSpec} or {\tt PredSpec}
  as {\tt Options} and {\tt PredSpec} is neither a variable, a
  predicate indicator, a callable term, or a comma-list or list of
  predicate indicators or callable terms.
\bi
...
\ei
\item {\tt Options} is not a dynamic option or a comma-list of dynamic
  options 
\bi
\item {\tt domain\_error(comma\_list\_or\_dynamice\_option,Options)}
\ei
\ei
\ei
