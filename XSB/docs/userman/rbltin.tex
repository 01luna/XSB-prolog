...
Each of the predicates in this section allows loading from files with
...
\standarditem{load\_dyn(+FileName,+Dir,+Module)}
...
\standarditem{load\_dync(+FileName,+Dir,+Module)}
{load\_dync/3}\label{load_dync/3}
Acts as {\tt load\_dync/2}, but loads the named file into the module named
{\tt Module}.

\standarditem{load\_dynca(+FileName)}{load\_dynca/1}
\index{canonical format}
Acts as {\tt load\_dync/1}, but loads predicates in the reverse order
of their appearance in the loaded file.  If the order of clauses
doesn't matter to the program, this is generally the fastest way to
load clauses.  It uses {\tt asserta/1} to add clauses to the dynamic
database, which avoids running index hash chains.  For files that
define predicates with clauses with very many hash collisions (for
example, if many clauses have the same value for an indexed argument),
use of this load predicate can provide significant speed up.
