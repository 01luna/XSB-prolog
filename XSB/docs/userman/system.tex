aa\chapter{System Description} \label{chap:system}
\label{system}
...
about: those directories in the dynamic loader path (see
...
that since directories in the dynamic loader path are searched in a
...
(e,g,, {\tt lib, syslib, cmplib} others), and finally in the current
working directory.  The exact directories and their order can be found by backtracking through the query:
...
\texttt{XSB\_USER\_AUXDIR} is set then
       ...
       library directories are searched by the dynamic loader {\em
...
code have advantages, as discussed further in Section~\ref{}.  Static
compiled code may be more optimized than asserted code, particularly
for clauses that have large bodies, but certain types of indexing,
...
via the predicates {\tt load\_dyn\_gen/2}, {\tt load\_dyn/[1,2]}, {\tt
  load\_dync/[1,2]}, {\tt ensure\_loaded/[2,3]} And other predicates
described in Section~\ref{sec:LoadDyn}.  These predicates act as {\tt
  consult/2} in that if a given file {\em File} has already been
dynamically loaded, old versions of predicates defined in {\em File}
will be retracted and their new definitions loaded to correspond to
those in {\em File} (except for predicates in which a {\tt
  multifile/1} declaration is present in {\em File}).  Dynamic loading
can be performed using XSB's reader of canonical terms (which does not
include operators or HiLog, but does allow list and comma-list
notation) via {\tt load\_dync/2} and {\tt load\_dynca/1}; dynamic
...
\bi
\item For large files, containing $10^5-10^8$ clauses, dynamic loading
  can be much faster than XSB's compiler, especially if the clauses do
  not need to be ordered, as with e.g., knowledge graphs.
%
\item Dynamically loaded files have advantages of dynamic code
  ...
  being modifiable via assert and retract. \footnote{Dynamically
    loaded code also allows larger atoms than statically loaded code.}
  %
  \item Dynamic loading fully supports module import and export,
    tabling declarations except for those mentioned below, and
    XPP-based preprocessing before load.  In addition it supports the
    use of operators and DCGs as long the dynamic load is not
    based on canonical reading.
    %
    \ei
...
\item Although dynamically loaded predicates are compiled into SLG-WAM
  ...
  %
\item Dynamic loading does not perform usage and definition checks for
  modules, or other static compilation checks such as singleton
  variable checks or checks of arithmetic expressions.
  %
...
%
\item[{\tt character\_set/1}] \index{compiler
  options!\texttt{character\_set/1}} The compiler option of {\tt
...
\item[{\tt modeinfer}]\index{compiler options!\texttt{modeinfer}}
	This option is used to trigger mode analysis. For each module
        compiled, the mode analyzer creates a {\tt {\em module}.D}
        file that contains the mode information that is inferred (or
        declared).
