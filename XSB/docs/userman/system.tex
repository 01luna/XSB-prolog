...
\item[{\tt xpp\_on/1}]
\item[{\tt xpp\_dump/1}]
  \index{GPP!\texttt{xpp\_on/1}}
  \index{GPP!\texttt{xpp\_dump/1}}
  %%
  XSB also allows one to pull program files through a series of external
...
%%
  ...
  the spec-sequence (or not appear at all), so it is possible to preprocess
  XSB files before and/or after (or instead of) GPP. Note that {\tt
    xpp\_on(gpp)}  and {\tt xpp\_dump(gpp)} are equivalent to {\tt xpp\_on}
  and {\tt xpp\_dump}, respectively. 

  To use a preprocessor other than GPP, two things must be done:
  %% 
  \begin{itemize}
  \item  A 4-ary Prolog predicate must be provided, which takes three
    arguments and produces a syntactically correct shell (Unix or Windows)
    command for invoking the preprocessor. The preprocessor should be
    taking its input either from the standard input or from a file and send
    the post-processed result to the standard output. The arguments to the
    shell command-producing predicate are as follows:
    %% 
    \begin{itemize}
    ...
    \item   Preprocessor name: this is the name under which the
      ...
      to be used to refer to the preprocessor (it does not need to be the
      same as the shell-command-producing predicate or the name of the
      preprocessor known to the OS).
    \item Options: these are the command-line
      options that the preprocessor might need. If the preprocessor spec
      mentioned above looks like {\tt foo(bar)}  then the
      preprocessor name would be bound to {\tt foo}  and Options to {\tt bar}.
    \item Shell command: this is the only output argument. It is supposed
      ...
      processed---that name will be added automatically as the last option
...
\end{verbatim}
%% 
    Here {\tt preproc\_name} is the user-given name of the preprocessor and
    {\tt preproc\_predicate} is the 4-ary shell-command-producing
    predicate described earlier.

    The registration query must be executed \emph{before} preprocessing 
    of the file is to start. Clearly, this implies that the
    ...
    second time the same name is used it is ignored. However, it is
    possible to register the same shell-command-producing predicate twice,
    if different preprocessor names are used.
  \end{itemize}
  %% 

  The difference between {\tt xpp\_on/1} and {\tt xpp\_dump/1}  
  is that the latter also saves the output of each preprocessor in a
  separate file. For instance, if the XSB file to be preprocessed
  is {\tt abc.P} and the {\tt xpp\_dump} option is
...
\end{verbatim}
  ...
  used by {\tt make\_append\_cmd/4}, when this predicate is called
  they will be bound to {\tt foobar.P}  
  and {\tt appendfile}, respectively, and could be used for various purposes. 
