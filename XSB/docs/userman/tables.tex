...
\subsection{Term Size Abstraction in XSB}
...
encountered.  If the size of $T_a$ exceeds the associate size limit
for $T$ (as declared in the next section), all further non-constant
functor symbols encountered in $T_i$ will be abstracted.  Once $T_a$
has been fully traversed, further arguments will be traversed in the
exact same manner.
...
Size-based abstraction as described above has been implemented in a
manner that has minimal if any impact on the speed of XSB's tabling
engine.  Note that by not abstracting functor symbols at depth 0 and
by abstracting each argument individually, both multi-argument
indexing and star indexing of subgoals will be usually be
preserved.\footnote{Other metrics could be used, such as term depth.
  Depth-based abstraction offers conceptual clarity; however
  size-based abstraction allows finer-grained optimization and incurs
  slightly less cost for XSB's tabling engine.}
