...
\index{termination!bounded rationality}
\section{Tabling for Termination}
%
...
only call other rules, possibly through negation ({\tt tnot/1} or {\tt
  sk\_not/1} in XSB); but where there are no calls to builtins for
...
body of that rule~\footnote{Evaluations that call non-ground negative
  ...
  avoided in most cases by using {\tt sk\_not/1}.}.
...
Abstraction can be performed for goals called positively; but not for
goals called negatively including {\tt tnot/1} and {\tt sk\_not/1};
rather, such goals will throw an exception if they surpass the
specified depth.  Subgoal abstraction is only implemented for call
variance, and currently is only settable globally, and not on a
predicate-by-predicate basis.
...
greater than {\tt max\_table\_subgoal\_depth}.  As an alternate to
setting flags, XSB can be called with the command-line arguments {\tt
  --max\_subgoal\_action a} and {\tt --max\_subgoal\_depth n} where
{\tt n} is the size of the desired depth.
...
are generated by a query.  For instance if the query {\tt p(X)} to the
program
%
\begin{verbatim}
p(f(X)) :- p(X).  
...
Semantically XSB is computing an approximation to the three-valued
...
is more than $n$, it is proven as usual.  Similarly, if $Q$ is false
in the well-founded model of a program, and none of the subgoals
...
sense that no incorrect answer will be derived, although sone answers
won't be known that might have been if the depth bound had been set
lower.  Due to the halting problem, there is no way to know in general
what depth to set for answer abstraction, or whether any bound needs
to be set at all.
...
{\tt get\_residual(A,Delay)} would bind {\tt Delay} to {\tt
  [brat\_undefined]}, where {\tt brat\_undefined/0} is simply a predicate defined as
...
\index{\texttt{explain\_u\_value/3}}
The XSB predicate

{\tt explain\_u\_val(+Answer,?Reason,?Type)}

\noindent
can be used to query why {\tt Answer} is undefined.  {\tt Reason} may
be either {\tt bounded\_rationality} or {\tt negative\_loops}.  {\tt
  Type} may be either {\tt direct} or {\tt indirect}.  {\tt
  bounded\_rationality} is considered direct if {\tt Answer} itself
has been abstracted; otherwise it is indirect.  {\tt negative\_loops}
is considered direct if {\tt Answer} is involved in a loop through
negation based on delay dependencies; otherwise it is considered
direct.  Thus, the predicate may succeed for more than one reason, but
for each reason there is at most one type,
...
\subsubsection{Using Bounded Rationality}
%
Bounded rationality is currently implemented only for tabling with
call variance, and lists are not currently included in the depth
check.  At this point bounded rationality is only settable globally,
and not on a predicate-by-predicate basis.

\index{Prolog flags!{\tt max\_table\_answer\_action}}
\index{Prolog flags!{\tt max\_table\_answer\_depth}}
%
Unless otherwise specified, XSB starts up with {\tt
  max\_table\_answer\_action} and {\tt
  max\_table\_answer\_list\_action} set to {\tt error} and {\tt
  max\_table\_answer\_depth} and {\tt max\_table\_answer\_depth} set
to the maximum integer possible under the compilation method for which
XSB has been configured.  Under this default behavior, XSB will throw
an error if a subgoal has depth greater than {\tt
  max\_table\_subgoal\_depth}.  Bounded rationality can be declared by
setting the Prolog flags
%
\bi
\item {\tt max\_table\_answer\_depth} to the desired maximal depth; and
\item {\tt max\_table\_answer\_action} to {\tt bounded\_rationality}.  
\ei
%
(cf. pg. \pageref{prolog-flags}). {\tt bounded\_rationality} is not
currently an accepted value for the flag {\tt
  max\_table\_answer\_list\_action}.

%\subsection{Summary of Techniques for Termination}
%
%%Apart from these two cases, a tabled evaluation will be finite and so
%will terminate.  Of course this is a theoretical argument, and
%termination could require a long time or a lot of memory.  For normal
%programs, the causes of non-termination turn out to be basically the
%same as for definite programs.  The causes of non-termination for
%programs that contain arithmetic builtins are more complex, and may
%depend on the behavior of the constraint domain used.
