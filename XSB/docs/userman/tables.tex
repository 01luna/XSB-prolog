...
A particularly intriguing use of term-sets is in conjunction with
...
sense that no incorrect answer will be derived, although some answers
...
indirect.  Thus, the predicate may succeed for more than one reason,
but for each reason there is at most one type.
...
where the bounded rationality depth has been set to 3, the query {\tt
  ...
  negative\_loops} and {\tt Type} to {\tt direct} and also {\tt
  Reason} to {\tt bounded\_rationality} and {\tt Type} to {\tt
  indirect}.  Note that {\tt p} also indirectly depends on {\tt q}
which is itself involved in a loop through negation.
\end{example}
...
greater than {\tt max\_table\_subgoal\_depth}.  Bounded rationality
can be declared by setting the Prolog flags
%
\bi
