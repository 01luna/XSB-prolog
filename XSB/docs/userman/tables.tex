...
\index{abstraction!subgoal}
\index{abstraction!answer}
...
{\em subgoal abstraction} and for bounded rationality through a type
of answer abstraction called {\em radial restraint}.
...
calls to built-in all-solutions predicates, or other
built-ins.  If such a fully-tabled pure normal program does {\em not}
have function symbols, XSB will always terminate for any query.  For
instance, XSB will terminate for fully tabled pure datalog programs --
even if the head of a rule is ``unsafe'' in that it contains variables
that do not occur in the body of that rule~\footnote{Evaluations that
  call non-ground negative literals will terminate through
  floundering, although this can be avoided in most cases by using
  {\tt not\_exists/1}.}.
...
or there can be an infinite number of answers~\footnote{Using the
  forest of trees model of tabling
  (cf. Section~\ref{sec:forest-trace}) non-termination requires that
  there are an infinite number of trees or that at least one tree has
  infinite size.}.
...
subsumption would help if the goal were {\tt ?- p(X)}).
...
\paragraph{Term Size Abstraction in XSB}
%
Both subgoal and answer abstraction in XSB are based on limiting the
size of a term.  The specific definition of size used is slightly
complicated, but offeres advantages discussed below.  A term $T$ is
traversed from left to right.  Each time a non-constant functor symbol
is encountered, the size is incremented by 1 -- regardless of the type
of symbol that is encountered.  Once the size limit has been reached,
all further non-constant functor symbols encountered will be
abstracted, with one important exception.  The functor symbols will be
abstracted {\em only} if they also occur at depth greater than 0.
...
is encountered.  Afterwards all non-constant functor symbols of depth
greater than 0 are abstracted.  This causes {\tt f/1} to be
abstracted, as it occurs at depth 1; however {\tt g/1} in the third
occurs at depth 0, and so is retained.  Similarly in the fourth
argument, the outer list symbol and head is preserved, while the tail
of the list is abstracted.
\end{example}

Size-based abstraction as described has been implemented in a manner
that has minimal if any impact on the speed of XSB's tabling engine.
Note that by not abstracting functor symbols at depth 0,
multi-argument indexing of subgoals will be usually be preserved.
However, XSB's star indexing will not always be preserved if it is
declared on an argument that is incuntered after a size limit has been
hit.\footnote{Other metrics could be used, such as term depth.
  Depth-based abstraction offers conceptual clarity and makes it
  easier to preserve star indexing; however size-based abstraction
  allows finer-grained optimization and incurs slightly less cost for
...
XSB performs subgoal abstraction based on the size limit described
above goals called positively; but not for goals called negatively as
this would create unsound negation.  Thus goals such as those those
inside {\tt tnot/1} and {\tt not\_exists/1} will throw an exception if
they surpass the specified term size. In addition, subgoal abstraction
is only implemented for call variance, {\em and applies equally all
  terms, whether they are lists or non-lists}.  However, despite these
restrictions, a tabled evaluation can be guaranteed to terminate for
queries to safe programs (cf.~\cite{RigS13}).
...
\item {\bf action} When a subgoal is encountered of maximum size,
  ...
  values for the action are {\tt error}. {\tt fail} and {\tt suspend}
  (cf. pg. \pageref{prolog-flags}).  \ei

\noindent
Unless otherwise specified, XSB starts up with {\tt
  max\_table\_subgoal\_action} set to {\tt error} and {\tt
  max\_table\_subgoal\_size} set to the maximum integer possible under
the compilation method for which XSB has been configured.  Under this
default behavior, XSB will throw an error if a subgoal has size
greater than {\tt max\_table\_subgoal\_size}.  As an alternative to
setting flags, subgoal abstraction can be set by calling XSB with the
...
\subsection{Bounded Rationality through Radial Restraint} \label{sec:restraint}
%
In XSB, a programmer can use a form of bounded rationality called {\em
  radial restraint} to ensure that only a finite number of answers are
generated by a query~\cite{GroS13}. For instance if the query {\tt
  p(X)} to the program
%
\begin{verbatim}
p(f(X)) :- p(X).  
...
\end{verbatim}
%
were evaluated using this approach with a size limit of 3, the
answers, {\tt p(0)}, {\tt p(f(0))}, {\tt p(f(f(0)))} and {\tt
  p(f(f(f(X))))} would be generated; {\em however}, {\tt
  p(f(f(f(X))))} would have the truth value of {\em undefined}.  Note
that by doing things in this way, both of the goals {\tt
...
XSB) bounding the size of an answer ensures that only a finite number
...
Semantically when radial restraint is used, XSB is computing an
...
that $Q$ is false.  Thus, the approximation that XSB computes is {\em
...
might have been if the size bound had been set higher.  Due to the
halting problem, there is no way to know in general what size to set
for answer abstraction, or whether any bound needs to be set at all.

If a restrained model is derived, answers that are undefined through
radial restraint can be desinguished from answers that are undefined
in the well-founded model of a program, or for other reasons such as
unsafe negation.  If an answer $A$ was abstracted due to a size
check, the query {\tt get\_residual(A,Delay)} would bind {\tt Delay}
to a list containing the atom {\tt brat\_undefined}, where {\tt
  brat\_undefined/0} is simply a predicate defined as
...
More generally, information about when a derivation has uses bounded
rationality can be obtained either thorugh the predicate {\tt
...
\subsubsection{Using Radial Restraint}
%
Radial restraint is currently implemented only for tabling with call
variance, and lists are not currently included in the size check.
%
...
$X = s(0)$ which has a size of only 1 and so this answer would
not be abstracted in the context of this subgoal.
\end{example}
...
%
Unless otherwise specified, XSB starts up with {\tt
  max\_table\_answer\_action} set to {\tt error} and {\tt
  max\_table\_answer\_size} set to the maximum integer possible under
the compilation method for which XSB has been configured.  Under this
default behavior, XSB will throw an error if a an answer has size
greater than {\tt max\_table\_answer\_size}.  %
