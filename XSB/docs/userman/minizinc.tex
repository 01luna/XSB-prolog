...
Constraint and optimization problems are specified un=sing the MiniZinc
...
\item
  ...
  \item    \texttt{MznF}: should be bound to a path to to the desired MznF
    model file (.mzn file) that contains a solution to a constraint or
  ...
  \item \texttt{OutTempl}: the template showing how the output should look
    ...
    \item  A term of the form \texttt{+}(\emph{atom}) or
      ...
      if it starts with a capital letter, it must be capitalized.

    \item A list of the form [$OutSpec_1$, ...,$OutSpec_n$]. Each
      \emph{Spec} in the list will be replaced according tho the rules
      described here.
    \item A term of the form $OutSpec_1$) \texttt{=} $OutSpec_2$. Each
      \emph{Spec} will be replaced according tho the rules described here.
    \end{itemize}
    %% 
  \item \texttt{Rslt}: must be a terms that matches the output template.
    Solutions to the constraints and optimization problems will be returned
    as bindings to this variable. They will also be asserted in
    \texttt{usermod}.
  \item \texttt{Xceptns}: a list of exceptions returned by the solver.
    ...
    variable would be bound to an empty list \texttt{[]}. Otherwise,
    ...
    \item  \texttt{unsatisfiable} --- the optimization problem is
      unsatisfiable.
    \item \texttt{unbounded}  --- the optimization problem has unbounded
      objective function. That is, in the problem is to maximize the
