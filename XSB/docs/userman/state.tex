...
\item{\em Tabling Properties}.  {\tt Property} unifies with properties
  from the the following set of categories only if the denoted
  predicate is tabled.
%
\begin{itemize}
\item The {\em Table Reuse Criterion} category, which is one of 
\begin{itemize}
\item{\tt [table\_reuse,variance]} The predicate has been declared
  tabled and to use call variance.
%
\item{\tt [table\_reuse,subsumption]} The predicate has been declared
...
\end{itemize}
%
\item The {\em Table Updating Behavior} category which can be one of 
\begin{itemize}
\item{\tt [table\_updating,incremental} The predicate was declared as
  either incremental dynamic or as incremental tabled; or
%
\item{\tt [table\_updating,opaque]} The predicate was declared as
...
\begin{itemize}
\item{\tt [subgoal\_size,N]}.  If {\tt N}$>1$, indicates the size at
  which tabled subgoals for the predicate are abstracted; if {\tt
    N}$=0$, indicates that subgoal abdstraction is not used for this
  predicate.
\end{itemize}
\item The {\em Answer Abstraction} category
\begin{itemize}
\item{\tt [answer\_size,N]}.  If {\tt N}$>1$, indicates the size at
  which tabled answers for the predicate are abstracted; if {\tt
    N}$=0$, indicates that answer abdstraction is not used for this
  predicate.
\end{itemize}
...
\item The {\em Answer Subsumption} property is active only if XSB is
  configured with multi-threading.  {\tt [answer\_subsumption,on]}
