...
\end{verbatim}
  ...
  reached, XSB can either throw a miscellaneous error (the default
  ...
  Note that this flag affects only structures that are not lists
...
\item {\tt max\_table\_answer\_depth} The maximum depth of an answer
  ...
  {\em Note that this flag affects only structures that are not lists
    (since large lists are more common than other large structures).
  }
...
\]
  ...
  incompatable with some constraint libraries such as CHR.  An
...
\item {\tt exception\_pre\_action} If set to {\tt
  print\_incomplete\_tables}, the predicate {\tt
  print\_incomplete\_tables/0} is called {\em before} throwing an
  exception, and this information can be used to help understand the
  context in which the exception arose.  Only exceptions that are
  thrown over at least one incomplete table will cause the file to be
  created.  (Figure out about file).  The default for this flag is
  {\tt off}.
%
