...
\item {\tt unknown} Indicates the behavior taken when calling an
  ...
  or {\tt error}, indicating that calls to unknown predicates should
  fail, produce a warning message to the {\tt user\_warning}
  stream. or throw an existence error.  The default setting is {\tt
    error}.
%
...
%
\item {\tt max\_table\_subgoal\_action} The action to take when a
  tabled subgoal of maximum size is encountered.  To understand the
  use of this flag, consider that if a predicate such as
\begin{verbatim}
p(X):- p(f(X)).
\end{verbatim}
  is tabled, it can create subgoals of unbounded size.  When the
  maximum subgoal depth is reached, XSB can
\begin{enumerate}
...
\item {\tt max\_table\_subgoal\_size} The maximum size of a subgoal
  argument that can be added to a table: when the size is reached, an
  action is taken as indicated for the previous flag.  The default
  value is the value of {\tt maximum\_integer}.

\item {\tt max\_table\_answer\_action} The action to take when a tabled
  answer of maximum size is encountered.  To understand the use of
...
\end{verbatim}
  ...
  this program will not terminate.  When the maximum answer size is
...
\item {\tt max\_table\_answer\_size} The maximum depth of an answer
