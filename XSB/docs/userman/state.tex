...
\item {\tt character\_set} If set to {\tt utf\_8}, interprets bytes as
  input to UTF-8 codepoints; if set to {\tt ascii}, then performs no
  transformations on input bytes.  Default for UNIX-style systems is
  {\tt utf\_8} and for Windows-style systems is {\tt ascii}, but the
  flag (and character sets) may be changed at any time.
