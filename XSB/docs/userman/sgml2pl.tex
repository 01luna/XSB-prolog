...
If the library is in a non-standard place, then the configure options
\texttt{--with-xpath-libdir=}{\em directory-of-libxml2} and
\texttt{--with-xpath-incdir=}\emph{libxml2-include-dir} must be given. The former
option must specify the directory of {\tt libxml2.so}  library, while
the later should specify the directory for the {\tt libxml2} include
files. It is the directory that contains
\texttt{include/libxml2}.
