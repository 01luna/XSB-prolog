...
\> \> assert( server\_id($Id: threads.tex,v 1.37 2008-03-08 18:12:11 pmoura Exp $) ),\\
\> \> thread\_join( $Id: threads.tex,v 1.37 2008-03-08 18:12:11 pmoura Exp $ ).\\
\\
\> server\_loop($SockFD$) :-\\
...
\>        $H$ is $N//2$, $H_1$ is $H + 1$.\\
\\
worker( $Q$, $Id: threads.tex,v 1.37 2008-03-08 18:12:11 pmoura Exp $, $I$, $F$, $List$, $Tail$) :-\\
\>        list\_of\_primes( $I$, $F$, $List$, $Tail$),\\
\>        thread\_send\_message( $Q$, primes($Id: threads.tex,v 1.37 2008-03-08 18:12:11 pmoura Exp $,$List$,$Tail$) ).\\
\\
master( $N$, $L$ )  :-\\
\>        partition\_space( $N$, $H$, $H_1$),\\
\\
