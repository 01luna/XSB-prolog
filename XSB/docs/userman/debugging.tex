...
\section{Understanding the State of a Tabled Computation} \label{sec:suspend-analyze}

These calls should be made while the current computation is suspended.

\index{Subgoal Dependency Graph (SDG)}
\index{affects relation}
\begin{description}
\ourmoditem{get\_sdg\_info(-SDG)}{get\_sdg\_info/1}{tables}
\index{Subgoal Dependency Graph (SDG)} 
%
For a suspended computation, returns information about the {\em
  Subgoal Dependency Graph} (SDG)~(cf. e.g. \cite{Swif99b,SwiW12}).
The vertices of the SDG are the tabled subgoals that are incomplete,
and a directed edge $(S_1,S_2)$ indicates that answers to $S_1$ are
directly called in evaluation of $S_2$ (i.e., $S_1$ directly affects
$S_2$).  The SDG is represented using an adjacency list that contains annotated
information about each subgoal.  Specifically, on success {\tt SDG}
will be instantiated to a list of terms of the form:

{\tt subgoal(Subgoal,SCCIndex,TableEntryHandle,CallsTo,Answers,Edges)} 

such that:

\bi
\item {\tt Subgoal} is an incomplete subgoal in the suspended computation
\item {\tt SCCIndex} is the index of the SCC to which the subgoal
  belongs.  For local evaluation, if $SCCIndex_1 < SCCIndex_2$, then
  subgoals in $SCCIndxex_2$ affect subgoals in $SCCIndex_1$ (although
  the affects relation may not be direct).
\item {\tt TableEntryHandle} is the handle to the table entry, and
  uniquely identifies {\tt Subgoal} 
...
\item {\tt Edges} is the atom {\tt unavailable} if {\tt Subgoal} is
  not incrementally tabled, otherwise it is a list of
  table entry handles for incomplete subgoals that {\tt Subgoal}
  directly affects.\footnote{This predicate is currently being
    extended to return this information for non-incremental tables as
    well, and to separate out positive from negative edges.}
\ei
...
%%\mbox{
%%{\epsfig{file=slg-forest-local,width=.99\textwidth}}}
\caption{SDG for {\tt ?- q(3,3)} when the computation is suspended by {\tt break/0}} \label{fig:sdg-break-1}
\end{figure}
%
...
\item  \ourmoditem{morph\_sdg(+SDGIn,+Morph,-SDGOut)}{morph\_sdg/3}{tables}
%
...
on {\tt In} and maps the Herbrand Base of the current program into
itself.  The syntax of {\tt SDGIn} and {\tt SDGOut} are the same, so
{\tt SDGOut} may be used for further morphisms; and accordingly {\tt
  SDGIn} may have been produced by {\tt get\_sdg\_info/1} and zero or
more applications of {\tt morph\_sdg/3}.
...
\]
while the edges of {\tt SDGOut}, $edges({\tt SDGOut})$ is the set
\[
\{ <morph(Subgoal_1),morph(Subgoal_2)> | <Subgoal_1,Subgoal_2> \in edges({\tt SDGIn}) \}
\]

The power of {\tt morph\_sdg/3} arises when the numbers of vertices
and edges of {\tt SdgIn} is large, and {\tt morph/2} ensures that
numbers for {\tt SDG} are much smaller -- thus allowing recognizable
patterns to emerge.  

For efficiency reasons, a specual condition is assumed about {\tt
  morph/2}: that if two elements of its range unify, then they must be
identical.  For instance, a morphism $M_1$ that reduced the maximum
depth of each non-variable argument of a term by 1 would not fit this
condition.  For instance $M_1(f(a,g(h(b)))) = f(X_1,g(h(X_2)))$ while
$M_1(f(a,g(b))) = f(X_1,g(X_2))$, which unify.  On the other hand, a
morphism that abstracts each argument to have a maximal fixed depth
would fulfill the condition.  As long as this condition is observed,
{\tt morph/2} may instantiated by an abstraction function as used
elsewhere in this manual: i.e., a function such that $morph(Term)$
subsumes $Term$.  However, other morphisms may also be useful as
demonstrated in Example~\ref{ex:morph-sdg} below.
...
{\tt subgoal(MorphSubg,null,Key,CallsTo,Answers,KeyList)} 
...
\item {\tt Key} is an atom identifying {\tt MorphSubg}.  Note that
  while each subgoal in {\tt SDGIn} corresponds to a tabled call, a
  given subgoal image in {\tt SDGOut} may not correspond to a tabled
  call in the current state.  Thus a table entry handle may not be
...
\item {\tt KeyList} is a list of the keys to those vertices adjacent
  to {\tt MorphSubg} as described above.
\ei

If there are no incomplete tables in the current state, an empty list is returned.
...
goal {\tt ?- morph\_sdg(SdgIn,mymorph,SdgOut} would produce:
...
{\bf Error Cases}  Need to figure out.

\item \ourmoditem{get\_sdg\_scc\_info(+SDG,-ListOut)}{get\_sdg\_scc\_info/2}{tables}

Given the SDG representation produced by {\tt
  get\_sdg\_info/1} this predicate returns information
about the SCCs that are currently under evaluation.  Upon success {\tt
  ListOut} will contain a term

{\tt scc(SCCIndex,NumSubgoals,NumAnswers,NumEdges)}
...
\item {\tt NumEdges} is the total number of edges within the SCC.
  \ei
