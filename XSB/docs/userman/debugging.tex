...
a powerful technique to understand the operational aspects of a
computation, including its termination
properties~\cite{LiaK13,LiaK13a}.  At the same time, it may not always
be convenient to use.  Since it is a trace-based analysis a (sometimes
very large) trace file must be created, loaded, and later analyzed.

\index{Subgoal Dependency Graph (SDG)} \index{affects relation} The
predicates in this section support certain types of analysis and
visualization of a tabled computation without a trace file, as long as
the computation has been suspended via {\tt timed\_call/2} or just a
ctrl-C interrupt, at which point they may be called from a break
level.  Specifically, these predicates return information about the
{\em Subgoal Dependency Graph} (SDG)~(cf. e.g. \cite{Swif99b,SwiW12}),
an abstract point of view about the state of a suspended computation.
The vertices of the SDG are the tabled subgoals that are incomplete;
and a signed directed edge $(S_1,S_2)$ indicates that answers to $S_1$
are directly called in evaluation of $S_2$ (i.e., $S_1$ directly
affects $S_2$), and indicates whether the relation is positive or
negative.  

In the predicates below, the SDG is represented using an adjacency
list that contains annotated information about each subgoal.  This
adjacency list is a list of terms of the form:
...
\item {\tt PosEdges} is a list of table entry handles for incomplete
  subgoals that {\tt Subgoal} positively directly affects.
\item {\tt NegEdges} is a list of table entry handles for incomplete
  subgoals that {\tt Subgoal} negatively directly affects.
 \ei
