...
{\tt trace/1} is like {\tt trace/0} except that it is non-interactive.
The trace log is dumped into the file provided as an argument.

\index[pred]{\texttt{trace/2}}
\index{\texttt{trace/2}}
{\tt trace/2} provides another form of non-interactive tracing.
It takes a file name in the first argument and an option in the
second. Currently the only supported option is \texttt{log}. 
Like \texttt{trace/1}, 
execution of \texttt{trace(somefile,log)} causes the log to go into the file
\texttt{somefile}. However, the log is written in the form of Prolog facts,
which can be loaded queried. The format of the facts is:
%% 
\begin{verbatim}
...
\texttt{CallNum} is the call number that XSB generates to show the
...
\texttt{ParentCallNum} is the cal number of the parent call.
\texttt{DepthOfCall} is the nesting depth of the current call with respect
to its ancestor calls. 
...
The name of the predicate (\texttt{xsb\_tracelog})
used for logging can be changed by asserting
it into the predicate \texttt{debug\_tracelog\_predicate/1}, which should be
imported from \texttt{usermod}. For instance,
%% 
\begin{verbatim}
