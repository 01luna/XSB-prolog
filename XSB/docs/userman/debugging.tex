...
\section{Understanding the State of a Tabled Computation} \label{sec:forest-trace}

{\bf Draft -- code is still being tested and is subject to change}

These calls should be made while the current computation is suspended.

\index{Subgoal Dependency Graph (SDG)}
\begin{description}
\ourmoditem{get\_incomplete\_subgoal\_info(-SDG)}{get\_incomplete\_subgoal\_info/1}{tables}

\index{Subgoal Dependency Graph (SDG)}
For a suspended computation, returns information about the {\em
  Subgoal Dependency Graph} (SDG)~(cf. e.g. \cite{Swif99b,SwiW12}).
The vertices of the SDG are the tabled subgoals that are
incomplete, and a directed edge $(S_1,S_2)$ indicates that answers to
$S_1$ are directly used to compute $S_2$ (i.e., $S_1$ directly affects
$S_2$).  The SDG is represented using an adjacency list that contains
extra information about each subgoal.  Specifically, on success {\tt
  SDG} will be instantiated to a list of terms of the form:
...
\item {\tt SCCIndex} is the index of the SCC to which the subgoal
  belongs.  For local evaluation, if $SCCInxex_1 < SCCInxex_2$, then
  subgoals in $SCCInxex_2$ affect subgoals in $SCCInxex_1$.
...
\item {\tt CallsTo} is the number of calls that have been made to the
  subgoal so far.  This number represents calls from both producer and
  consumer subgoals.
\item {\tt Answers} is the number of answers that {\tt Subgoal} has so far.
\item {\tt Edges} is the atom {\tt unavailable} if {\tt Subgoal} is
  not incrementally tables, otherwise it is a list of
  TableEntryHandles for incomplete subgoals that {\tt Subgoal}
...
\item \ourmoditem{get\_incomplete\_scc\_info(+SDG,-ListOut)}{get\_incomplete\_scc\_info/2}{tables}

Given the SDG representation produced by {\tt
  get\_incomplete\_subgoal\_info/1} this predicate returns information
