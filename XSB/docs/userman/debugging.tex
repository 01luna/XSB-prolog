...
\centering
\mbox{
{\epsfig{file=slg-forest-local.eps,width=.99\textwidth}}}
...
\item {\tt cmp} if $S_1$ is not a new subgoal but has been completed
...
\item {\em Location of errors} Whenever an error is thrown and the
  ...
  the form {\tt err(SCounter)} is logged, where {\em Counter} is the
