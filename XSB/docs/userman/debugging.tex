...
To turn on tracing, use {\tt trace/0} or {\tt trace/1}.  To turn tracing off, use {\tt
notrace/0}.  
\begin{description}
...
All Prolog prompts also go into that file until {\tt notrace/0} is typed. 
...
\standarditem{debug}{debug/0}
    ...
    The predicates {\tt trace/0}, {\tt trace/1},  and {\tt spy/1} cause debugging mode
...
\index{trace/1}
{\tt trace/1} is like {\tt trace/0}, except that it is non-interactive: the
trace is dumped into the file provided as an argument.  
