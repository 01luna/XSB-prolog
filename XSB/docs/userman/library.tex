...
\ourmoditem{set\_arg(+Index,+Term,+Value)}{machine}\index{\texttt{set\_arg/3}}
%
...
\ourmoditem{epoch\_seconds(-Seconds)}{machine} 
\ourmoditem{epoch\_milliseconds(-Seconds,-Milliseconds)}{machine} 
...
\begin{description}
\ourmoditem{profile\_unindexed\_calls(+Par)}{xsb\_profiling}\index{profile\_unindexed\_calls/1}
...
\ourmoditem{load\_csv(+FileName,+PredSpec)}{proc\_files} \index{\texttt{load\_csv/2}}
...
\ourmoditem{load\_dsv(+FileName,+PredSpec,+Options)}{proc\_files} \index{\texttt{load\_dsv/3}}
...
\ourmoditem{pretty\_print(+Stream,+ClausePairs)}{pretty\_print}
...
\ourmoditem{module\_of\_term(+Term,?Module)}{machine}
