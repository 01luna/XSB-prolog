...
A PDB is stored as a trie, which is a ground Prolog term.  Each
branching point in the trie is implemented by a variant of a radix
tree.  A set of clauses is canonically represented, i.e., no matter
what sequence of operations, such as {\tt assert\_in\_db}'s, {\tt
  retractall\_in\_db}'s, or {\tt union\_db}'s, that one uses to
construct a particular set of clauses, the resulting PDBs (i.e.,
Prolog terms) are identical.~\footnote{Since a Prolog DB is a term and
  must be passed as an argument through the predicates that use it, it
  differs from other implementations of tries in XSB, such as interned
  tries (cf. Chapter~\ref{chap:tries}), or trie-indexed facts
  (cf. Section~\ref{sec:assert}), which are persistent.}
...
A PDB represents an unordered set of clauses.  The order in which
clauses are returned from {\tt clause\_in\_db} (and thus for {\tt
  call\_in\_db}) is indeterminate.
...
distributed as:
<XSB\_INSTALL\_DIR>/lib/prolog\_db\_doc/prolog\_db.pdf.)
