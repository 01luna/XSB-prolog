...
with spreadsheets, Python, Java, and other tools.  XSB has a library
{\tt proc\_files} to support I/O of Prolog predicates to
delimiter-separaed files. \footnote{The name of the library, {\tt
    proc\_files} is unfortunately vague, but it is too late to change
  this name.}
...
In this form {\tt Arity} must equal the number of fields in {\tt File}
file, and {\tt PredSpec} is inferred to be dynamic.  Each line in the
file will define one fact of {\tt PredSpec}.  In accordance with csv
formatting, any filed in {\tt File} enclosed in double quotes will be
treated as a single field, and thus can contain commas and new-lines.
Any clauses for {\tt PredSpec} will be retracted before the facts from
the file are added.  With this simple form of predicate specification,
each field will be loaded as an atom (including fields that contain
just integers.)

If more control over typing is needed, {\tt PredSpec} may be of the
form {\tt predName(TypeSpec1,...P,TypeSpecN)}, and each {\tt
  TypeSpec$_I$} indicates the type of the corresponding field in {\tt
...
%
  ...
  \texttt{date\_converter/2} could be a predicate that takes atoms of
  the form \texttt{'2017-11-26'} in the first argument and binds the
  second argument to terms of the form \texttt{date(2017,11,26)}.To
  remove a type conversion hook, use
...
above.  {\tt Options} is a list of options.  (With an empty {\tt
  Options} list, {\tt load\_dsv} acts as {\tt load\_csv/2}.)  The
options are:
\begin{itemize}
...
\item[{\tt delimiter=''C''}] (or {\tt delimiter(''C'')}) which
  ...
  field delimiter, the default being the double quote, ``'''''' (and
  I've yet to find a situation in which I want to change it).
...
\item[{\tt returns=''C''}] (or {\tt returns(''C'')}, where {\tt C} is
  a string, causes the lines that have newlines in non-delimited
  fields to be processed specially.  The system tries to recover from
  such situations and process such fields correctly.  If a newline (or
...
\item[{\tt reverse}] causes {\tt asserta/1} to be used to assert the
  input terms, so the predicate will contain the file records in
  reverse order.  This can significantly improve performance if
  indexing is particularly weak.
...
with arguments 1 and 2 being unbound and distinct. For instance, a
hook could be a predicate of the form
...
associated with @var{Type}.  If no hook is associated with @var{Type},
succeeds silently.
...
{\tt load\_dyn/1} or dynamically loaded predicates.
...
\ourmoditem{save\_dsv(+FileName,PSpec,Options)}{save\_dsv/3}{proc\_files}
Writes out a separated/delimited file from a Prolog predicate.
{\tt FileName} is the name of the file to create.  {\tt PSpec}
...
Options are as in {\tt load\_dsv/3}.
...
represention for sets of terms where an efficient subset operation is
...
{\tt sym\_diff\_db/3} returns in {\tt DB3} the symetric difference of
the sets of clauses in {\tt DB1} and {\tt DB2}.  I.e., the result set
contains clauses in {\tt DB1} but not in {\tt DB2} and clauses in {\tt
  DB2} but not in {\tt DB1}.
