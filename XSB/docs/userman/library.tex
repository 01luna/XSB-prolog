...
finds them and generating {\tt document\_import}/{\tt
  document\_export} declarations that might be useful.  It can be used
after a large multi-file, multi-module XSB program has been written to
find possible inconsistencies in (or interesting aspects of) how
predicates are defined and used.
...
\item
   ...
   that is exported but never used.  This can be useful to see if
   predicate could be deleted.  Again this option produces many
   warnings for predicates in multi-use libraries.
\item
...
% TES -- I couldn't understand this, so I tried rewriting it below.
