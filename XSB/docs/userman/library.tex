...
\begin{description}
\ouroldmoditem{append(?List1, ?List2, ?List3)}{basics}\index{\texttt{append/3}}
%\predindex{append/3~(L)}
    Succeeds if list {\tt List3} is the concatenation of lists 
    {\tt List1} and {\tt List2}.

\ouroldmoditem{member(?Element, ?List)}{basics}\index{\texttt{member/2}}
...
\ouroldmoditem{memberchk(?Element, ?List)}{basics}\index{\texttt{memberchk/2}}
...
\ouroldmoditem{ith(?Index, ?List, ?Element)}{basics}\index{\texttt{ith/3}}
%\predindex{ith/3~(L)}
...
\ouroldmoditem{delete\_ith(+Index, +List, ?Element, ?RestList)}{listutil}
...
\ouroldmoditem{log\_ith(?Index, ?Tree, ?Element)}{basics}\index{\texttt{log\_ith/3}}
...
\ouroldmoditem{log\_ith\_bound(?Index, ?Tree, ?Element)}{basics}
...
\ouroldmoditem{length(?List, ?Length)}{basics}\index{\texttt{length/2}}
...
\ouroldmoditem{same\_length(?List1, ?List2)}{basics}\index{\texttt{same\_length/2}}
...
\ouroldmoditem{select(?Element, ?L1, ?L2)}{basics}\index{\texttt{select/3}}
%\predindex{select/3~(L)}
    {\tt List2} derives from {\tt List1} by selecting (removing) an 
    {\tt Element} non-deterministically.

\ouroldmoditem{reverse(+List, ?ReversedList)}{basics}\index{\texttt{reverse/2}}
...
\ouroldmoditem{perm(+List, ?Perm)}{basics}\index{\texttt{perm/2}}
%\predindex{perm/2~(L)}
...
\ouroldmoditem{subseq(?Sequence, ?SubSequence, ?Complement)}{basics}
...
\ouroldmoditem{merge(+List1, +List2, ?List3)}{listutil}\index{\texttt{merge/3}}
%\predindex{merge/3~(L)}
...
\ouroldmoditem{absmerge(+List1, +List2, ?List3)}{listutil}\index{\texttt{absmerge/3}}
    Predicate {\tt absmerge/3} is similar to {\tt merge/3}, except that 
    it uses predicate {\tt absmember/2} described below rather than 
    {\tt member/2}.

\ouroldmoditem{absmember(+Element, +List)}{listutil}\index{\texttt{absmember/2}}
...
\ouroldmoditem{member2(?Element, ?List)}{listutil}\index{\texttt{member2/2}}
...
\ouroldmoditem{closetail(?List)}{listutil}\index{\texttt{closetail/1}}
...
\ouroldmoditem{comma\_to\_list(+CommaList,-List)}{basics}
\index{\texttt{comma\_to\_list/2}}
%
Transforms {\tt CommaList} to {\tt List}.

\ournewrepeatitem{comma\_append(?CL1,?CL2,?CL3)}{basics}
\index{\texttt{comma\_append/3}}
%
\ournewrepeatitem{comma\_length(?CommaList,?Length)}{basics}
\index{\texttt{comma\_length/2}}
%
\ournewrepeatitem{comma\_member(?Element,?CommaList)}{basics}
\index{\texttt{comma\_member/2}}
%
\ouroldmoditem{comma\_member(?Element,?CommaList)}{basics}
...
\begin{description}
\ouroldmoditem{get\_attr(-Var,+Mod, ?Val)}{machine}\index{\texttt{get\_attr/3}} 
%
...
\ouroldmoditem{put\_attr(-Var,+Mod, ?Val)}{machine}\index{\texttt{put\_attr/3}}
%
...
\ouroldmoditem{del\_attr(-Var,  +Mod)}{machine}\index{\texttt{del\_attr/2}} 
%
...
\ouroldmoditem{install\_attribute\_portray\_hook(Module,Attribute,Handler)}{machine}
...
\ouroldmoditem{install\_attribute\_constraint\_hook(Module,Vars,Names,Handler)}{machine}
...
\begin{description}
\ouroldmoditem{when(+Condition,Goal)}{constraintLib}\index{\texttt{when/2}}
%
...
\ouroldmoditem{unifiable(X, Y, -Unifier)}{constraintLib}
...
\ouroldmoditem{setarg(+Index,+Term,+Value)}{constraintLib}\index{\texttt{setarg/3}}
\ouroldmoditem{set\_arg(+Index,+Term,+Value)}{machine}\index{\texttt{set\_arg/3}}
%
...
\ouroldmoditem{term\_variables(+Term,-Variables)}{constraintLib}
...
\begin{description}
\ouroldmoditem{format(+String,+Control)}{format}\index{\texttt{format/2}}
\ouroldmoditem{format(+Stream,+String,+Control)}{format}\index{\texttt{format/3}}
...
%\ouroldmoditem{current\_input\_port(-IOport)}{curr\_sym} \index{\texttt{current\_input\_port/}}
%   See {\tt current\_output\_port/1}.
%\ouroldmoditem{current\_output\_port(-IOport)}{curr\_sym} \index{\texttt{current\_output\_port/}}
...
%\ouroldmoditem{file\_open(+FileName,+Mode,-IOport)}{file\_io} \index{\texttt{file\_open/3}}
...
%
%\ouroldmoditem{file\_close(+IOport)}{file\_io}\index{\texttt{file\_close/1}}
%    Closes the file (or string) for descriptor {\tt IOport}.

%\ouroldmoditem{ioport2iostream(+IOport, -Stream)}{file\_io}
...
\ouroldmoditem{file\_write(+IOport,+Term)}{xsb\_writ}
\index{\texttt{file\_write/2}}
    Writes the term {\tt Term} to the file (or string) with descriptor {\tt
    IOport}.

\ouroldmoditem{file\_read(+IOport,-Term)}{xsb\_read}
...
\ouroldmoditem{file\_read(+IOport,-Term,-Vars)}{xsb\_read}
...
\ouroldmoditem{file\_read\_canonical(+IOport,-Term,-Psc)}{machine}
...
%\ouroldmoditem{fget\_line(+Str,-Inlist,-Next)}{scrptutl}
...
\altourmoditem{str\_sub(+Sub, +Str)}{str\_sub/3}{string}
...
\altourmoditem{str\_match(+Sub, +Str, +Direction, ?Beg, ?End)}{str\_match/5}{string}
...
\ouroldmoditem{str\_length(+Str, ?Result)}{string}
...
%%
\ouroldmoditem{substring(+String, +BeginOffset, +EndOffset, -Result)}{string}
...
\begin{description}
\ouroldmoditem{expand\_filename(+FileName,-ExpandedName)}{machine}
...
\ouroldmoditem{expand\_filename\_no\_prepend(+FileName,-ExpandedName)}{shell}
...
\ouroldmoditem{parse\_filename(+FileName,-Dir,-Base,-Extension)}{machine}
...
\ouroldmoditem{sleep(+Seconds)}{shell}
\index{\texttt{sleep/1}}
...
\ouroldmoditem{sys\_pid(-Pid)}{shell}
\index{\texttt{pid/1}}
Get Id of the current process.

\ouroldmoditem{getenv(+VarName,-VarVal}{machine} 
...
\ouroldmoditem{putenv(+String)}{machine} 
...
\ouroldmoditem{epoch\_seconds(-Seconds)}{machine} 
\ouroldmoditem{epoch\_milliseconds(-Seconds,-Milliseconds)}{machine} 
\index{\texttt{epoch\_seconds/1}}
\index{\texttt{epoch\_milliseconds/2}}
%
...
\ouroldmoditem{file\_time(+FileName, -time(Time1,Time2))}{file\_io}
...
\ouroldmoditem{file\_size(+FileName, -Size)}{file\_io}
\index{\texttt{file\_size/2}}

Returns file size.

\ouroldmoditem{cwd(-Dir)}{shell}
\index{\texttt{cwd/1}}
Get current working directory.

\ouroldmoditem{is\_absolute\_filename(+FileName)}{machine} 
...
\ouroldmoditem{cd(+Path)}{shell}
\index{\texttt{cd/1}}
Change directory.

\ouroldmoditem{rename(+Old,-New)}{shell}
\index{\texttt{rename/2}}
Rename file.

\ouroldmoditem{directory(+Path,?Directory)}{directry}
...
\ouroldmoditem{file\_to\_list(S, List)}{scrptutls}
...
%% Do not use it!
%%
%%\ouroldmoditem{sysin(+Command,?Output)}{scrptutl}
%%\index{\texttt{sysin/2}}
%%
...
%
\ouroldmoditem{get\_process\_table(-ProcessList)}{shell}
...
%%
\begin{description}
\ouroldmoditem{fd2ioport(+Pipe, -IOport)}{shell}\index{\texttt{fd2ioport/2}} 
...
\begin{description}
\ouroldmoditem{array\_new(-Array,+Size)}{array}\index{\texttt{array\_new/2}}
%
Creates a one dimensional empty array of size {\tt Size}.  All the
elements of this array are variables.  

\ouroldmoditem{array\_elt(+Array, +Index, ?Element)}{array} \index{\texttt{array\_elt/3}} 
%
Succeeds iff {\tt Element} unifies with the {\tt Index}-th element of
array {\tt Array}.  
%
\ouroldmoditem{array\_update(+Array, +Index, +Elem)}{array} \index{\texttt{array\_update/3}} 
%
...
\begin{description}
\ouroldmoditem{profile\_call(+Goal)}{xsb\_profiling}\index{profile\_call/1}
...
\begin{description}
\ouroldmoditem{profile\_mode\_call(+Goal)}{xsb\_profiling}\index{profile\_mode\_call/1}
...
\begin{description}
\ouroldmoditem{profile\_mode\_dump}{xsb\_profiling}\index{profile\_mode\_dump/0}
...
\begin{description}
\ouroldmoditem{profile\_mode\_init}{xsb\_profiling}\index{profile\_mode\_init/0}
...
\begin{description}
\ouroldmoditem{profile\_unindexed\_calls(+Par)}{xsb\_profiling}\index{profile\_unindexed\_calls/1}
...
\begin{description}
  %%
\ouroldmoditem{new\_trie(-Root)}{intern} \index{\texttt{newtrie/1}}
        {\tt Root} is instantiated to a handle for a new trie.

\ouroldmoditem{trie\_intern(+Term,+Root)}{intern}
...
\ouroldmoditem{trie\_interned(?Term,+Root)}{intern}
...
\ouroldmoditem{delete\_trie(+Root)}{intern} \index{\texttt{delete\_trie/1}}
%%
...
\begin{description}
  %%
\ouroldmoditem{trie\_intern(+Term,+Root,-Leaf,-Flag,-Skel)}{intern}
...
\ouroldmoditem{trie\_intern(+Term,-Leaf,-Skel)}{intern}\index{\texttt{trie\_intern/3}}
%%
Acts as {\tt trie\_intern/5}, but interns {\tt Term} into the default
trie and does not return the new/old flag.

\ouroldmoditem{trie\_interned(?Term,+Root,?Leaf,-Skel)}{intern}
...
\ouroldmoditem{trie\_interned(?Term,?Leaf,-Skel)}{intern} \index{\texttt{trie\_interned/3}}
%%
Similar to {\tt trie\_interned/4}  but uses the default trie.

\ouroldmoditem{trie\_unintern(+Root,+Leaf)}{intern} \index{\texttt{trie\_unintern/2}}
%%
...
\ouroldmoditem{trie\_unintern\_nr(+Root,+Leaf)}{intern}\index{\texttt{trie\_unintern\_nr/2}}
%%
...
\ouroldmoditem{unmark\_uninterned\_nr(+Root,+Leaf)}{intern}\index{\texttt{unmark\_uninterned\_nr/2}}
...
\ouroldmoditem{reclaim\_uninterned\_nr(+Root)}{intern}
...
\begin{description}
\ouroldmoditem{prepare(+Index)}{gensym} \index{\texttt{prepare/1}}
%
...
\ouroldmoditem{gennum(-Var)}{gensym} \index{\texttt{gennum/1}}
%
Unifies {\tt Var} with a new integer.

\ouroldmoditem{gensym(+Atom,-Var)}{gensym} \index{\texttt{gensym/2}}
%
...
\ouroldmoditem{random(-Number)}{random} \index{\texttt{random/1}}
%
Binds \texttt{Number} to a random float in the interval [0.0, 1.0).
Note that 1.0 will never be generated.

\ouroldmoditem{random(+Lower,+Upper,-Number)}{random} \index{\texttt{random/3}}
...
\ouroldmoditem{getrand(?State)}{random} \index{\texttt{getrand/1}}
...
\ouroldmoditem{setrand(rand(+X,+Y,+Z))}{random} \index{\texttt{setrand/1}}
...
\ouroldmoditem{datime\_setrand}{random} \index{\texttt{datime\_setrand/0}}
...
\ouroldmoditem{randseq(+K, +N, -RandomSeq)}{random} \index{\texttt{randseq/3}}
...
\ouroldmoditem{randset(+K, +N, -RandomSet)}{random} \index{\texttt{randset/3}}
...
\ouroldmoditem{gauss(-G1,-G2)}{ramdom} \index{\texttt{gauss/2}}
%
...
\ouroldmoditem{weibull(K,Lambda,X)}{ramdom} \index{\texttt{weibull/3}}
%
...
\ouroldmoditem{exponential(K,X)}{ramdom} \index{\texttt{exponential/2}}
%
...
\ouroldmoditem{load\_csv(+FileName,+PredSpec)}{proc\_files} \index{\texttt{load\_csv/2}}
...
\ouroldmoditem{load\_dsv(+FileName,+PredSpec,+Options)}{proc\_files} \index{\texttt{load\_dsv/3}}
...
\ouroldmoditem{scan(+List,-Tokens)}{stdscan} \index{\texttt{scan/2}}  
...
\ouroldmoditem{scan(+List,+FieldSeparator,-Tokens)}{stdscan} 
\index{\texttt{scan/3}}  
...
\ouroldmoditem{set\_scan\_pars(+List)}{stdscan} 
...
\ouroldmoditem{get\_scan\_pars(-List)}{stdscan} 
...
\ouritem{retractall\_in\_db(+ClauseHead,+DB0,-DB)}
...
\ouritem{clause\_in\_db(?ClauseHead,?ClauseBody,+DB)}
...
\ouroldmoditem{term\_hash(+Term,+HashSize,-HashVal)}{machine}
...
\ouroldmoditem{pretty\_print(+ClausePairs)}{pretty\_print}
\ouroldmoditem{pretty\_print(+Stream,+ClausePairs)}{pretty\_print}
\index{\texttt{pretty\_print/1}}
\index{\texttt{pretty\_print/2}}
%
...
\ouroldmoditem{module\_of\_term(+Term,?Module)}{machine}
\index{\texttt{module\_of\_term/2}}
%
