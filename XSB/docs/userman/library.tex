...
loads a csv-formatted file into memory so that it defines the
predicate {\tt PredSpec}.  The simplest form of {\tt PredSpec} is {\tt
  PredName/Arity}.  In this case {\tt Arity} must equal the number of
...
new-lines.)  Any clauses for {\tt PredSpec} will be removed before the
facts from the file are added.  WIth this simple type of predicate
...
{\tt TypeSpec$_I$} indicates the type of the corresponding field in the
