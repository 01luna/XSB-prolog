...
\item[{\tt string}] The corresponding field value will become a string
  (a list of ascii code integers as produced from an atom by {\tt
  atom\_codes/2}) in the loaded fact.
...
\begin{itemize}
\item[{\tt separator=''Sep''}] (or separator(''Sep'')) which indicates
  that the character(s) {\tt Sep} will be used as the field separator.
  There may be one or more characters.

  {\tt Example: } To load a tab-separated file (tsv) use 
    \verb|separator="\t"| in the options list.
\item[{\tt delimiter=''C''}] (or delimiter(''C'')) which indicates
  that the single character {\tt C} will be used as the field
  delimiter, the default being the double quote, ``'''''' (and I've
  yet to find a situation in which I want to change it).
...
\item[{\tt returns=''C''}] (or returns(''C''), where {\tt C} is a
  string, causes the lines that have newlines in non-delimited fields
  to be processed specially.  The system tries to recover from such
  situations and process such fields correctly.  If a newline (or
  return) is detected in a field, it will be replaced with the string
  {\tt C} in the output field.
\item[{\tt reverse}] causes asserta to be used to assert the input
  terms, so the predicate will contain the file records in reverse
  order.  This can significantly improve performanc if indexing is
  particularly weak.
\item[{\tt pad=$N$}] (or {\tt pad($N$)}, where $N$ is a (positive or
  negative) integer.  If $N$ is negative, then the asserted predicate
  must have $|N|$ more fields than the input records contain, and they
  will be padded with terms of the form {\tt 'NULL'(\_\_)}.  If $N$ is
  positive, then the input records must contain $N$ more fields than
  the asserted predicate, and the extra fields in the records will be
  ignored.
\end{itemize}
...
{\tt RecTerm}.  The details of how it works depend on the value of
{\tt Format}.  The {\tt FileName} denotes a file, and is found by
looking through the {\tt file\_directory/1} directors, just as for
{\tt load\_dyn/1} or dynmaically loaded predicates.

The permissible values for {\tt Format} are:
\begin{itemize}

\item[{\tt read}] indicates that the file contains Prolog terms that
  can be read with Prolog's {\tt read/1} predicate, so the terms can
  use operators.  In this case {\tt RecTerm} will be unified with each
  term that is read.  So normally {\tt RecTerm} is a variable in this
  case.
...
\item[{\tt dsv(Options)}] where {\tt Options} is a list of options as
  described for {\tt load\_dsv/3} above.  In this case, this predicate
  will read records (as specified by {\tt Options}) from a
  delimited-separated file, just as are loaded by {\tt load\_dsv/3}.
  In this case {\tt RecTerm} will be unified with a list of values in
  each record (in turn.)

\item[{\tt sql(??)}] reserved for future functionality.
\end{itemize}
  
A common use of this predicate is as in the following example:
\begin{verbatim}
:- table_index(employee/6,[1]).
...
\end{verbatim}
Here we define a predicate {\tt employee/6} whose first field is the
file from which to get the employee records, and whose 2nd through 6th
fields contain the data about a given employee.  The {\tt
  table\_index} directive declares that on all calls to employee/6,
the first argument ([1] indicating here the {\tt FileName}) must be
bound.  On the first call, the call will be abstracted to having just
the first argument bound, and that call will be made and
(subsumptively) tabled.  Every subsequent call will retrieve its
answer(s) from that table.  So this essentially loads the data from
the indicated file into the predicate on first call, and from then on
uses that table.

One can get behavior very similar to {\tt load\_dync(FileName)} where
file {\tt FileName} contains facts for a single predicate, say pp/3,
with:

\begin{verbatim}
:- table_index(pp_f/4,[1]).

pp_f(FileName,F1,F2,F3) :-
    data_records(FileName,canonical,pp(F1,F2,F3)).
\end{verbatim}

where {\tt pp\_f/4} contains the data in the file, every tuple having
the filename as its first field.  Note that since {\tt table\_index/2}
uses trie indexing, the {\tt FileName} is stored only once in the
trie, and the records are appropriately indexed.  (See the
documentation for {\tt table\_index/2} for more details and how to
handle more situations requiring more complex indexing.)

Notice that this way of loading data from files is declarative; there
is no need to have an ``initialization'' operation that must be called
to load the data and then shouldn't be called again.  Here, given the
above declaration, the programmer simple calls the predicate (e.g.,
{\tt pp\_f/4}) whenever she needs those values, and the system
automatically loads the data on the first call, building the necessary
indexes.  This can greatly simplify programming with data from files.
...
\section{``Pure'' Meta-programming and Set Processing in XSB with prolog\_db.P}
...
rules, and herein called a {\em Prolog DB}), and then ask for a goal
to be proved in such a Prolog DB.  A Prolog DB can also be used to
manipulate sets of facts (or rules), as it provides efficient set
operations of union, intersection, subset, etc.
...
intern these DB's, and to table them (as intern), or to use {\tt
  intern\_termhash/2} explicitly with them.

Prolog DB's, as sets of terms, can be combined with the binary set
operations of union, intersection, and set difference, and they can be
compared for subset and disjointness.  These operations take advantage
of the trie structure of the DB representation, using common
substructures of the input DB's where possible, to be quite efficient.
Thus it can be effictive to maintain distinct databases and then
combine them with set union, intersection, or difference.  Also this
is a good represention for sets of terms where an efficient subset
operation is required, perhaps with aggregation and answer
subsumption, when compbined with {\tt intern\_termhash/2}.
...
DB, {\tt DB0}, and returns a new Prolog DB, {\tt DB}.  {\tt Clause}
may be a list of clauses, in which case they will all be added.  A
Prolog DB is a {\em set} of clauses, so asserting a clause that is
already in {\tt DB0} just returns that same database.  No ordering of
clauses is preserved, so cuts do not make sense and cannot be used in
clauses.  (The if-then else ('->'/3) should be used instead.)
...
{\tt DB0} unify, then {\tt DB0} is returned unchanged. {\tt
  ClauseHead} may be a list of clause heads, in which case they will
each be used in turn to remove clauses from {\tt DB0}.
...
clauses in {\tt DB1} and {\tt DB2}.  This operation takes advantage of
the structure of the representations of the input {\tt DBs} to share
...
{\tt DB} will fully share the represention of that predicate from the
other {\tt DB} (and not even look at its contents.)
...
sets of clauses in {\tt DB1} and {\tt DB2}.  Again, the result shares
...
{\tt disjoint\_db/2} succeeds if no clause in both {\tt DB1} and {\tt
  DB2}, i.e., considered as sets of clauses, {\tt DB1} and {\tt DB2}
are disjoint sets.
