...
%
{\tt get\_residual/2} backtracks through the answer set of each
...
The information is provided at two levels of aggregation.  If {\tt
  info(summary)} is an element of {\tt OptionList}, the aggregate
number of subgoals and answers are given that are subsumed by {\tt
  Term}. If {\tt info(details)} the actual subgoals are returned along
with the number of answers and their listing is also returned.  In
either case, if {\tt Term} is a variable this information is broken
down by tabled predicates.

By default {\tt table\_dump/2} writes to the current output stream.
If {\tt results(X)} is in {\tt OptionList}, {\tt X} is instantiated to
a list of terms representing summary and/or detail information.  {\tt
  table\_dump/3} provides for output to be written to a given stream.
...
%
\begin{verbatim}
| ?- table_dump(_X,[info(summary)]).

summary = q(A) - subgoals(0) - returns(0).
summary = r(A) - subgoals(0) - returns(0).
summary = p(A,B) - subgoals(0) - returns(0).
summary = s(A,B) - subgoals(0) - returns(0).

yes
| ?- table_dump(_X,[info(details)]).

details - undefined - subgoals(0) - answers_for_subgoals([]).
details - q(A) - subgoals(1) - answers_for_subgoals([q(B) - 2 - [q(2),q(1)]]).
details - r(A) - subgoals(0) - answers_for_subgoals([]).
details - p(A,B) - subgoals(3) - answers_for_subgoals([p(C,D) - 5 - 
      [p(3,1),p(3,2),p(2,b),p(1,b),p(1,a)],p(2,b) - 
      2 - [p(2,b),p(2,b)],p(1,a) - 2 - [p(1,a),p(1,a)]]).
details - s(A,B) - subgoals(0) - answers_for_subgoals([]).
...
\item {\tt OptionList} contains an element, {\tt O}, that is not a
  valid {\tt table\_dump\_option}, i.e., {\tt info(summary)}, {\tt
    info(details)} or {\tt results(Var)} 
\bi
