...
%In terms of
%implementation, ``the table'' is actually a set of mini-tables, each
%one containing entries for a particular predicate.  
...
\begin{example}
Let $S =$ \code{p(X,f(Y))} be a generaator subgoal.  Using a return
...
\ourstandarditem{get\_calls(\#CallTerm,-TableEntryHandle,-ReturnTemplate)}{get\_calls/3}{Tabling}
%
...
template is constructed in the third.  These latter two arguments must
be uninstantiated at call time.  The error terms are the same as for
{\tt get\_calls/1}.
...
\ourstandarditem{get\_returns(+TableEntryHandle,\#ReturnTemplate)}{get\_returns/2}{Tabling}
%
...
predicate such as {\tt get\_call/3} or {\tt get\_calls/3}.
...
further instantiated as well as \code{DelayList}.
...
\index{tabling!conditional answers} \index{\texttt{get\_residual/2}}
...
\ourrepeatstandarditem{abolish\_table\_subgoal(+Subgoal,+Options)}{abolish\_table\_subgoal/2}{Tabling}
