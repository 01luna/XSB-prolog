...
%will leave dangling pointers in the data structures used for uncremental updates.  
%
...
deletes the table structures for {\tt CallTerm}, but pointers to {\tt
  CallTerm} in the Incremental Dependency Graph (IDG), and invalidates
all subgoals that depend on {\tt CallTerm}. The node and edges for
{\tt CallTerm} will be reinserted into the IDG when {\tt CallTerm} is
re-evaluated, either lazily or by an explicit update command.
...
%
Abolishes all tables that are {\em not} declared to be incremental.
Such a predicate allows XSB to function in a manner similar to that
of a deductive database: incremental tables will be automatically
updated when the data they depends on changes; while non-incremental
tables, which may have become invalid, can be abolished.
...
Note that as currently implemented, {\tt
  abolish\_nonincremental\_tables/1} traverses through each tabled
predicate, $Pred$, and if  $Pred$ has any incomplete subgoals, no
subgoals for $Pred$ will be abolished; rather {\tt
  abolish\_nonincremental\_tables/1} will skip over such subgoals,
even if they are completed.

{\bf Error Cases} Error cases for incomplete
tables are described above; additionally: 
\bi
\item {\tt Options} is a variable, or contains a variable as an element
\bi
\item {\tt instantiation\_error}
\ei
\item {\tt Options} contains an option that is not {\tt on\_incomplete(skip)} or {\tt on\_incomplete(error)}
\bi
\item {\tt domain\_error(abolish\_nonincremental\_tables/1,}
{\tt [on\_incomplete(skip), on\_incomplete(error)])}
\ei
\ei
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
