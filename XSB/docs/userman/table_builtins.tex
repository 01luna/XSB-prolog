...
\end{tabbing}
%
where {\em Pred/Goal} and $N_{subgoals}$ are as above and $List$ has teh form 
%
\begin{tabbing}
fooo\=foo\=foo\=foo\=fooooooooooooooooooooooooooooooo\=ooooooooooooo\=\kill
%
\>  Subgoal - times\_called($N_{called}$) - answers($N_{answers}$) - status - List\_of\_Answers
%
\end{tabbing}
%
...
all those answers currently derived for $Subgoal$.  Finally $status$ is
%
\bi
...
\ourmoditem{print\_completion\_stack}{print\_completion\_stack/0}{tables}
%
This predicate, which can be useful for debugging purposes, prints out
to {\tt stddbg} each subgoal in XSB's completion stack, followed by
the ordinal number of the subgoal.  The completion stack gives a
notion of the dependencies among tabled predicates.  In local
evaluation (the default evaluation method for XSB) all predicates in
SCC $m$ depend on predicates in SCC $n$ if $m < n$~\footnote{This
...
%
Note that in addition to the dependencies discussed above, all
subgoals in SCC 1 depend on one another.
