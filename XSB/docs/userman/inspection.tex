...
This indicates that there are 1065417 subgoals (complete or incomplete)
...
\item {\em max\_table\_subgoal\_size}. 
\bi  
\item {\em Limit:} The maximum size of a given argument in a
 subgoal (cf. Section~\ref{sec:size-metric}).  
\item{\em  Possible Actions:} {\tt abstract}, {\tt suspend} or {\tt error}.
\item {\em Default.} Limit: {\tt MAX\_INT}; Action: {\tt error}.
\ei
...
\bi  
\item {\em Limit:} The maximum number of subgoals that are
 incomplete, and are in the same SCC at any given time.
\item{\em  Possible Actions:} {\tt suspend}, {\tt error} or {\tt warning}.
\item {\em Default.} Limit: {\tt MAX\_INT}; Action: {\tt error}.
\ei
...
\bi  
\item {\em Limit:} The maximum size of a given argument of an
 answer (cf. Section~\ref{sec:size-metric}).  
\item{\em  Possible Actions:} {\tt abstract}, {\tt suspend}, or {\tt error}.
\item {\em Default.} Limit: {\tt MAX\_INT}; Action: {\tt error}.
\ei

\item {\tt max\_answers\_for\_subgoal} (Not yet implemented)
\bi  
\item {\em Limit:} The maximum number of for any single tabled
 subgoal.
\item{\em  Possible Actions:} {\tt suspend}, {\tt error} or {\tt warning}.
\item {\em Default.} Limit: {\tt MAX\_INT}; Action: {\tt error}.
\ei
...
\item {\em Limit:} If an integer, the limit is the maximum
 ...
 machine on which XSB is running.
