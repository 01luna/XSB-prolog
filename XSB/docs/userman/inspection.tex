...
{\tt ?- set\_prolog\_flag(tripwire(max\_scc\_subgoals),parameter(100)).}
...
\index{\texttt  {timed\_call/2}}
\index{\texttt  {timed\_call\_modify/1}}
%
...
\item {\em There are a potentially unbounded number of tabled
  ...
  be applied based on term size either globally through the tripwire
  {\tt max\_table\_subgoal\_size}, or on a predicate-by-predicate
  basis through tabling directives
  (cf. Section~\ref{sec:tabling-termination}).  In other words, when a
  tabled subgoal, $S_{deep}$, is called whose size is greater then
