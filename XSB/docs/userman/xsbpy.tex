...
As with many machine-learning tooks, Faiss expects that each of the
...
\begin{itemize}
\item {\tt faissInit(+XbFile,+Dim)} initizlizes a Faiss index where
  {\tt XBFile} is a text file containing the vectors to be indexed and
  {\tt Dim} is the dimension of these vectors. ({\tt xb} is Faiss
  terminology for the set of indexed vectors.)  This predicate also
  creates a {\tt numpy} array with a set of query vectors {\tt xq}
  consisting of the same vectors.  With the query and index vectors
  both set up in this manner, the vector space can be explored to
  understand which of its elements are suitable for clustering, and so
  on.

  After execution of this predicate, a fact for the predicate {\tt
    xp\_faiss:xq\_num/1} contains the number of query vectors (xq),
  which is the same as the number of indexed vectors (xb).

\item {\tt get\_k\_nn(+Node,+K,-Neighbors)} finds the K nearest
...
translate automatically. \footnote{The examples {\tt xp\_fasttext} and
  {\tt googleTrans} were written by Albert Ki; {\tt xp\_faiss} was
  written by ALbert Ki and Theresa Swift.}
