...
A configuration process of {\tt xsbpy} has been written for Linux, Mac
...
versions 3.5 -- 3.10 of Python and its libraries.\footnote{The {\tt
  ...
  been tested on Windows 10; on Ubuntu, Fedora and CentOs Linuxes; and on
  on Intel-based Mac.}  The {\tt xsbpy} configuration script is called
by the main XSB configuration script, and can be found in {\tt
  XSB/packages/xsbpy}.  {\tt xsbpy} configuration can be run
separately from the main configuration script if desired.

 and Mac testing, 
\subsection{Configuring {\tt xsbpy} under Linux with GCC} \label{sec:xsbpy-linux}
...
\begin{itemize}
\item git and gcc (or some other compiler) must be present and usable by the
  user performing the configuration.

\item As discussed below, a version of C-Python 3.x (the usual
  implementation of Python) with development extensions must be
  present and usable.
...
\subsubsection{Installing {\tt xsbpy} to work with Anaconda}

Anaconda comes with all of the Python development extensions needed
for {\tt xsbpy}.  To make use of them, simply activate your conda
environment, (re)configure and (re)make XSB as described in the
manual.
...
for the user "warren".  You must adjust the example paths to correctly
...
{\tt C:/Users/warren/AppData/Local/Programs/Python/Python39/libs/python3.lib}.
...
To run see if the interface is installed corrected and functional, it
...
\subsection{Space Management for XSB's Heap} \label{sec:xsbpy-memory}
...
provides an interface to Facebook's dense vector managment system
Faiss.  The distinction between the two is that Faiss can manage
vectors read in from a file, and provides batch-oriented operatoins;
...
\begin{description}
  ...
  Returns a the vecotr for {\tt Word} in the model {\tt Name} as a
  Prolog list of floats.  In general, if a compuation on word vectors
