...
\noindent
{\large {\bf {\em This chapter is a draft documentation of
...
provides an efficient and easy way for XSB to call Python 3 functions
...
has been tested using versions 3.5 -- 3.8 Python libraries.

\subsection{Configuring {\tt xsbpy} under Linux with GCC}

In principle, when XSB is configured on Linux, {\tt xsbpy} will also be
configured so that it can be used like any other XSB module.\footnote{
  If several versions of Python 3.x are installed, the most recent will be
...
\item A development version of Python 3.x must also be present and
  usable by the user performing the configuration.  This version
  must be a CPython version (this is the usual implementation of
  Python), In addition the version of Python must also contain a
...
\noindent
the latest shared-object library must also be downloaded as, e.g.:
...
XSB process, the core Prolog modules of {\tt xsbpy} are loaded into
...
{{\em The translation of JSON through xsbpy, although it is
...
\end{verbatim}
%%
Note that \verb|''| here is an empty string (Prolog 0-length atom). 
...
experience so far, many Python libraries can be called directly and will
...
\subsection{Translating Between RDF and Prolog: xp\_rdflib}
